...
{{-- Blade helper function to determine image URL --}}
@php
    ...
    function getImageUrl($path) {
        ...
        // For new paths (e.g., 'developers/...') or previous 'officials/pictures/...' or 'officials/icons/...'
        // These paths are relative to storage/app/public, so prepend '/storage/'
    ...
    <div class="flex flex-col md:flex-row gap-8 justify-center items-center w-full min-h-screen p-4 md:p-8">
        @foreach ($officials as $official)
            <div class="relative grid h-[40rem] w-full max-w-[28rem] flex-col items-end justify-center overflow-hidden rounded-xl bg-white bg-clip-border text-center text-gray-700
                ...
                <div class="absolute inset-0 m-0 h-full w-full overflow-hidden" style="background-image: url('{{ getImageUrl($official->picture) }}'); background-size: cover; background-position: center;">
...
{{-- This specific image path is also updated for consistency --}}
<img class="absolute bottom-[-50px] right-0 w-[736px] h-64 object-contain mb-10" src="{{ asset('storage/miniflagv2.svg') }}" alt="Philippine Flag Colors Decoration" />
